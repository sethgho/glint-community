...
import { validateManifest, REQUIRED_EMOTIONS, MAX_FILE_SIZE } from '../lib/package-spec';
...
/** Download individual emotion (SVG or rasterized PNG) */
styles.get('/:author/:slug/emotions/:emotion', async (c) => {
  ...
  const svgPath = join(uploadsDir, author, slug, style.version, `${emotion}.svg`);
  
  if (!existsSync(svgPath)) {
    return c.json({ error: 'SVG file not found on disk' }, 500);
  }
  
  // Serve SVG directly if requested
  if (format === 'svg') {
...
styles.post('/', requireAuth, requireScope('publish'), async (c) => {
  ...
  // Extract emotion SVG files
  const emotions = new Map<string, Buffer>();
  
  for (const emotionName of manifest.emotions) {
    const file = formData.get(emotionName);
    if (!file || !(file instanceof File)) {
      return c.json({ error: `Missing SVG file for emotion: ${emotionName}` }, 400);
    }

    if (file.size > MAX_FILE_SIZE) {
      return c.json({ error: `SVG too large for ${emotionName}: ${file.size} > ${MAX_FILE_SIZE}` }, 400);
    }
    ...
    if (!content.includes('<svg')) {
      return c.json({ error: `${emotionName} is not a valid SVG file` }, 400);
    }
    
    // Verify hash matches manifest
    const hash = createHash('sha256').update(buffer).digest('hex');
    const expectedHash = manifest.files[`${emotionName}.svg`];
