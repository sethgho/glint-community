...
web.get('/', async (c) => {
  ...
  c.header('Cache-Control', user ? 'private, no-cache' : 'public, s-maxage=3600, max-age=300');
  return c.html(html);
});
...
web.get('/contribute', async (c) => {
  const user = c.get('sessionUser');
  c.header('Cache-Control', user ? 'private, no-cache' : 'public, s-maxage=86400, max-age=3600');
